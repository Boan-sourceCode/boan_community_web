...
<template>
	<div class="app-container">
		<!-- 查询条件 -->
		<el-form :model="queryParams" ref="queryParams" size="small" :inline="true" label-width="82px">
			<el-form-item label="所属组织：" prop="partyId">
				<treeSelect v-model="queryParams.partyId" clearable :props="partyTreeProps" :options="partyTreeData" :value="value" placeholder="请选择">
				</treeSelect>
			</el-form-item>
			<el-form-item label="年份：" prop="year">
				<el-date-picker v-model="queryParams.year" type="year" value-format="yyyy" placeholder="选择年">
				</el-date-picker>
			</el-form-item>
			<el-form-item label="性别：" prop="sex">
				<el-select clearable v-model="queryParams.sex" placeholder="请选择">
			...
			<el-form-item style="float: right">
				<!--<el-button type="primary" plain @click="clearExport()">重置</el-button>-->
				<el-button type="primary" plain @click="handleExport()">导出</el-button>
			</el-form-item>
		</el-form>
		<!-- 表格 -->
		<el-table :loading="loading" :data="partyMemberList" :header-cell-style="{ background: '#EBF7FF' }" stripe border>
			...
			<el-table-column label="优秀党员年份" align="center" prop="year">
				<template slot-scope="scope">
					<p v-if="scope.row.year === '' || scope.row.year === null">/</p>
					<p v-else>{{scope.row.year}}</p>
				</template>
			</el-table-column>
		...
		<!-- 编辑弹窗 -->
		<el-dialog :visible.sync="editDialog" title="优秀党员编辑" width="70%" :close-on-click-modal="false" @close="dictEditVisible('editForm')" center>
			<template>
					...
					<div class="dialogBox">
						...
						<el-form-item label="证件类型：" prop="certificateType" class="input-width">
							<el-select clearable disabled v-model="editForm.certificateType" placeholder="请选择">
								<el-option v-for="dict in dict.type.certificate_type" :key="dict.value" :label="dict.label" :value="dict.value">
								</el-option>
							</el-select>
						</el-form-item>
						...
						<el-form-item label="出生日期：" prop="birthday" class="input-width">
							<el-date-picker disabled v-model="editForm.birthday" type="date" value-format="yyyy-MM-dd" placeholder="选择日期">
							</el-date-picker>
						</el-form-item>
						...
						<el-form-item label="所属党支部：" prop="partyId" class="input-width">
							<treeSelect disabled v-model="editForm.partyId" :props="partyTreeProps" :options="partyTreeData" :value="value" placeholder="请选择">
							</treeSelect>
						</el-form-item>
						...
						<el-form-item label="党员状态：" prop="partyMemberStatus" class="input-width">
							<el-select clearable v-model="editForm.partyMemberStatus" placeholder="请选择">
								<el-option v-for="dict in dict.type.party_status" :key="dict.value" :label="dict.label" :value="dict.value">
								</el-option>
							</el-select>
						</el-form-item>
						<el-form-item label="申请入党时间：" prop="applyJoinpartyTime" class="input-width">
							<el-date-picker v-model="editForm.applyJoinpartyTime" type="date" value-format="yyyy-MM-dd" placeholder="选择日期">
							</el-date-picker>
						</el-form-item>
						<el-form-item label="正式入党时间：" prop="officiaJoinpartyTime" class="input-width">
							<el-date-picker v-model="editForm.officiaJoinpartyTime" type="date" value-format="yyyy-MM-dd" placeholder="选择日期">
							</el-date-picker>
						</el-form-item>
...
<script>
	import { partyMember, partyTreeLimit, partyEdit, partyPost, partySubmit } from "@/api/area/partymember";
	export default {
		name: "partyerGood",
		dicts: [
			"sys_user_sex",
		...
		methods: {
			...
			getList() {
				this.loading = true;
				partyMember(this.queryParams).then((res) => {
					this.partyMemberList = res.rows;
...
//			},
			...
			//导出按钮操作
			handleExport() {
				this.download('/system/memberHonor/export', {
					...this.queryParams
				}, `优秀党员-${new Date().getTime()}.xlsx`)
			},
