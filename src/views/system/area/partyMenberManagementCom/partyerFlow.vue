...
<template>
  <div class="app-container">
    ...
    <el-form
      ...
      :inline="true"
      label-width="82px">
      <el-form-item label="所属组织：" prop="beforePartyBranch">
        <treeSelect
          v-model="queryParams.beforePartyBranch"
    ...
    <el-table
      ...
      <el-table-column label="原属组织" align="center" prop="beforePartyName">
        <template slot-scope="scope">
          <p v-if="scope.row.beforePartyName === '' || scope.row.beforePartyName === null">/</p>
          <p v-else>{{scope.row.beforePartyName}}</p>
        </template>
      </el-table-column>
      <el-table-column label="现属组织" align="center" prop="partyName">
        <template slot-scope="scope">
          <p v-if="scope.row.partyName === '' || scope.row.partyName === null">/</p>
          <p v-else>{{scope.row.partyName}}</p>
        </template>
      </el-table-column>
      <el-table-column label="转入说明" align="center" prop="transoutReason">
        <template slot-scope="scope">
          <p v-if="scope.row.transoutReason === '' || scope.row.transoutReason === null">/</p>
          <p v-else>{{scope.row.transoutReason}}</p>
        </template>
      </el-table-column>
      <el-table-column label="转入时间" align="center" prop="createTime">
        <template slot-scope="scope">
          <p v-if="scope.row.createTime === '' || scope.row.createTime === null">/</p>
...
<script>
import { flowParty,partyTreeLimit,partyEdit,partyPost,partySubmit } from "@/api/area/partymember";
export default {
  name: "partyerFlow",
  dicts: [
    "sys_user_sex",
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      flowParty(this.queryParams).then((res) => {
