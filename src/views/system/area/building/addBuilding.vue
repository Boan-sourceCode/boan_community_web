	...
	<div class="app-container">
		...
		<nav>
					...
					<div class="card_wrap">
							...
							<ul style="margin-bottom: 30px">
								<li>
									...
									<el-select v-model="stepOne.architectureType" placeholder="请选择">
										<el-option v-for="dict in dict.type.building_type" :key="dict.value" :label="dict.label" :value="dict.value" @click.native="Buildingpulldown(dict)">
										</el-option>
									</el-select>
								</li>
								<li>
								...
								<li>
									...
									<el-select v-model="stepOneAndTwoParams.communityName" placeholder="请选择">
										<el-option v-for="(item, index) in belongCommunity" :key="index" :label="item.communityName" :value="item.communityId" @click.native="getCommunityInfo(item)">
										</el-option>
									</el-select>
								</li>
								<li>
						...
						<div class="card_center card_top">
							<ul>
								<li>
									...
									<div>
										<vue-tags-input style="width: 547px" placeholder="回车键添加" v-model="buildingNumber" :tags="tags" @tags-changed="newTags => tags = newTags" />
									</div>
								</li>
								<li>
									<span class="essential">*</span><span>地上楼层数：</span>
									<el-input v-model="stepOne.storyNumber" placeholder="请输入内容" type="number" onKeypress="return (/[\d]/.test(String.fromCharCode(event.keyCode || event.which))) || event.which === 8"></el-input>
								</li>
								<li>
									<span>地下楼层数：</span>
									<el-input v-model="stepOne.undergroundNumber" placeholder="请输入内容" type="number" onKeypress="return (/[\d]/.test(String.fromCharCode(event.keyCode || event.which))) || event.which === 8"></el-input>
								</li>
							</ul>
						...
						<div class="card_bottom card_top">
							<ul style="margin-top: 16px">
								<li>
									<span class="essential">*</span><span>单元名称：</span>
									<!--                  <el-input class="one" v-model="unit" placeholder="请输入内容"></el-input>-->
									<div>
										<vue-tags-input style="width: 547px" placeholder="回车键添加" v-model="unit" :tags="unitTags" @tags-changed="newTags => unitTags = newTags" />
									</div>
								</li>
								<li>
									<span><span class="essential">*</span>单元每层用户数：</span>
									<el-input v-model="stepOne.unitPerfloorHousenumber" type="number" placeholder="请输入内容" onKeypress="return (/[\d]/.test(String.fromCharCode(event.keyCode || event.which))) || event.which === 8"></el-input>
								</li>
							</ul>
							<span class="info">单元+计量单位，例如：1单元，2单元；A单元，B单元</span>
						</div>
					</div>
				</el-card>
			</div>
				...
				<el-card class="box-card">
					...
					<div class="step2_four">
						...
						<div class="select_wrap">
							<span>楼栋号</span>
							<ul>
								<li v-for="(item, index) in listOne" :key="index" :class="activeOne === index ? 'isActive' : ''" @click="selectActiveOne(item, index)">
						...
						<div class="select_wrap">
							<span>单元</span>
							<ul>
								<li v-for="(item, index) in listTwo" :key="index" :class="activeTwo === index ? 'isActive' : ''" @click="selectActiveTwo(item, index)">
									<span v-if="item.buildingInfo !== []">{{item.unitNumber}}</span>
								</li>
							</ul>
						</div>
						<div class="select_wrap">
			...
			<el-card class="step3_wrap" v-if="active === 3">
				...
				<div class="step2_four">
					<div class="select_wrap">
						<span>楼栋号</span>
						<ul>
							<li v-for="(item, index) in listThree" :key="index" :class="activeOne === index ? 'isActive' : ''" @click="stepThreeBuildActive(item, index)">
								<span>{{ item.buildingName }}</span>
								<!--                <div class="unit_wrap">
                  <span>单元</span>
                  <ul>
                    <li v-for="(unit, index) in item.buildingInfo" :key="index" :class="activeTwo === index ? 'isActive' : ''"
                        @click="stepThreeUnitActive(unit, index)">
                      <span>{{unit.unit}}</span>
                    </li>
                  </ul>
                </div>-->
					...
					<div class="select_wrap">
						<span>单元</span>
						<ul>
							<li v-for="(unit, index) in stepThreeUnitList" :key="index" :class="activeTwo === index ? 'isActive' : ''" @click="stepThreeUnitActive(unit, index)">
								<span>{{unit.unit}}</span>
							</li>
						</ul>
					</div>
					<div class="select_wrap">
						<span>层数</span>
						<ul>
							<li v-for="(item, index) in stepThreeFloorList" :key="index" :class="activeThree === index ? 'isActive' : ''" @click="stepThreeLayers(item, index)">
				...
				<el-card>
					...
					<div class="step2_four">
						<div class="select_wrap">
							<span>楼栋号</span>
							<ul>
								<li v-for="(item, index) in listThree" :key="index" :class="activeOne === index ? 'isActive' : ''" @click="selectActiveOne(item, index)">
									<span>{{ item.buildingName }}</span>
								</li>
							</ul>
						</div>
						<div class="select_wrap">
							<span>单元</span>
							<ul>
								<li v-for="(unit, index) in stepThreeUnitList" :key="index" :class="activeTwo === index ? 'isActive' : ''" @click="selectActiveTwo(unit, index)">
									<span>{{unit.unit}}</span>
								</li>
							</ul>
						</div>
						<div class="select_wrap">
							<span>层数</span>
							<ul>
								<li v-for="(item, index) in stepFourFloor" :key="index" :class="activeStepThree === index ? 'isActive' : ''" @click="stepFourLayers(item, index)">
									<span>{{item.storey}}</span>
								</li>
							</ul>
						</div>
                  ...
                  />-->
						...
						<div class="select_wrap_last">
							...
							<el-select class="select" v-model="stepFiveParams.houseNumber" placeholder="请选择">
								<el-option v-for="dict in dict.type.house_no" :key="dict.value" :label="dict.label" :value="dict.value">
								</el-option>
							</el-select>
						</div>
					</div>
			...
			<div class="step5_wrap parent" v-if="active === 5">
				<el-table :data="stepFiveTableList" border style="width: 100%" :header-cell-style="{background:'#EBF7FF',color:'#606266', 'text-align': 'center'}">
					<el-table-column prop="id" label="房屋编号" width="100" align="center" :header-row-style="{background:'#000'}">
					</el-table-column>
					<el-table-column prop="buildingName" label="楼栋号" width="180" align="center">
					</el-table-column>
					<el-table-column prop="unit" label="单元" width="180" align="center">
					</el-table-column>
					<el-table-column prop="storey" label="楼层" width="180" align="center">
					</el-table-column>
					<el-table-column prop="houseNumber" label="房屋门牌号" width="180" align="center">
					</el-table-column>
					<el-table-column prop="houseAddress" label="房屋标准地址">
				...
				<!-- 分页 -->
				<pagination :total="total" :page.sync="stepFiveParams.currentPage" :limit.sync="stepFiveParams.pageSize" @pagination="stepFiveCreateTable()" />
			</div>
		</nav>
		<footer>
			<el-button class="previous" style="margin-top:12px;" v-if="active === 2" @click="previous" type="info">上一步</el-button>
			<el-button class="previous" style="margin-top:12px;" v-if="active === 4" @click="previous" type="info">上一步</el-button>
			<el-button class="previous" style="margin-top:12px;" v-if="active === 5" @click="previous" type="info">上一步</el-button>
		...
		<!-- 步骤四点击更多弹出遮罩层 -->
		<el-dialog :visible.sync="dialogEdit" title="门牌号" width="800px" :close-on-click-modal="false">
			<div class="dialog-card-wrap">
				<p>{{ this.buildingTile + ',' + this.unitTitle + ',' + this.storeyTitle }}：</p>
...
<script>
	import {
		...
		getReturnGridInfo,
		createHouseNumber,
		createDefaultHouseNumber,
		createUpdateHouseNumber,
		updateFloorNumber,
		updateHouseNumber,
		createHouseTable,
		stepFiveUpdateAddress,
		batchInsertHouse,
	...
	export default {
		...
		methods: {
			...
			stepThreeBuildActive(item, index) {
				...
				// debugger;
				this.stepThreeFloorList = this.stepThreeUnitList[index].floor
				this.activeTwo = 0
				this.activeThree = 0
