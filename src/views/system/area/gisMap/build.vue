    ...
    <el-tabs v-model="activeName" tabPosition="right" class="tabRight" @tab-click="handleClick">
      ...
      <el-tab-pane name="type">
        <div slot="label" @click="isShow">
          <i class="icon-molds iconRight" style="background:#fff"></i>
          <p style="color: #1F79F3">建筑类型</p>
        </div>
      </el-tab-pane>
    </el-tabs>
    <!--内容-->
      ...
      <div class="contentRight">
        ...
        <div v-if="this.activeName === 'type'">
          <div style="padding: 0 20px">
            ...
            <div>
              <el-select clearable v-model="buildType" placeholder="请选择" style="width: 100%"
                @change="getBuildPurpose($event)">
            ...
            <div class="category">
              <p :class="index === active ? 'isActive' : ''" v-for="(item, index) in purposeOptions" :key="index"
                @click="getPurposeList(item, index)">
                {{ item.buildingPurpose }}
              </p>
            </div>
          </div>
          <div>
            <p class="title" v-if="this.purposeList.length !== 0 && this.purposeName !== ''">搜索 “{{ this.purposeName }}”
              结果{{ this.typePage.total }}条</p>
            <ul id="purposeList"></ul>
            <!-- 分页 -->
            <pagination small background :hide-on-single-page="show" layout="prev, pager, next" :total="typePage.total"
              :page.sync="typePage.pageNum" :limit.sync="typePage.pageSize" @pagination="getPurposeList()" />
          </div>
        </div>
        <div v-if="this.activeName === 'search'">
          <div style="padding: 0 20px">
            ...
            <div>
              <el-input clearable v-model="searchName" placeholder="请输入建筑名称" style="margin-right: 10px;width: 72%;" />
              <el-button class="searchButton" @click="getSearch">搜索</el-button>
            </div>
          </div>
          <div>
            <p class="title" v-if="this.searchList.length !== 0 && this.buildingName !== ''">搜索 “{{ this.buildingName }}”
              结果{{ this.searchPage.total }}条</p>
            <ul id="myList"></ul>
            <!-- 分页 -->
            <pagination small background :hide-on-single-page="show" layout="prev, pager, next" :total="searchPage.total"
              :page.sync="searchPage.pageNum" :limit.sync="searchPage.pageSize" @pagination="getSearch()" />
          </div>
        </div>
      </div>
    </div>
  ...
  methods: {
    ...
    getSearch() {
      ...
      build(param).then((res) => {
        ...
        this.buildingName = this.searchName
        this.searchPage.total = res.total
        let searchMap = this.$parent.map
        let that = this
        for (var i = 0; i < this.searchList.length; i += 1) {
          //处理坐标数据
          if (this.searchList[i].gis !== null) {
            var centerArray = this.searchList[i].gis.split(',');
          }
          let center = []
          centerArray.forEach((item) => {
            let number = Number(item)
        ...
        AMapUI.loadUI(['misc/MarkerList', 'overlay/SimpleMarker', 'overlay/SimpleInfoWindow'],
          function (MarkerList, SimpleMarker, SimpleInfoWindow) {
            ...
            that.searchMarkerList = new MarkerList({
              //关联的map对象
              map: searchMap,
              //列表的dom容器的id
              ...
              getInfoWindow: function (data, context, recycledInfoWindow) {
                if (recycledInfoWindow) {
                  ...
                  recycledInfoWindow.setInfoBody('<p style="font-size: 12px;margin: 4px">所属网格：' + data.area_name + '</p>' +
                    ...
                    '</div>' +
                    '<div>' +
                    '<p style="font-size: 12px;margin: 4px;display: inline-block;width: 47%;color: #FF933E;">租贷房屋：' + data.autonomicNum + '</p>' +
                    '<p style="font-size: 12px;margin: 4px;display: inline-block;width: 47%;color: #333333;">空置房屋：' + data.vacantNum + '</p>' +
                    '</div>' +
                    '<p style="font-size: 12px;margin: 4px">地址：' + data.building_address + '</p>',);
                  return recycledInfoWindow;
            ...
            //监听鼠标选中改变
            that.searchMarkerList.on('selectedChanged', function (event, info) {
              if (info.selected) {
                if (info.selected.marker) {
                  //更新为选中样式
                  info.selected.marker.setIconStyle(selectedIconStyle(info.selected.index));
            ...
            //监听鼠标悬停改变
            that.searchMarkerList.on('listElementMouseenter markerMouseover', function (event, record) {
              if (record && record.marker) {
                //非选中的id
                if (!this.isSelectedDataId(record.id)) {
                  //设置为hover样式
            ...
            //监听鼠标移出改变
            that.searchMarkerList.on('listElementMouseleave markerMouseout', function (event, record) {
              if (record && record.marker) {
                if (!this.isSelectedDataId(record.id)) {
                  //恢复默认样式
                  record.marker.setIconStyle(defaultIconStyle(record.index));
            ...
            //展示数据
            that.searchMarkerList.render(that.searchList);
          });
        this.remove()
      })
    },
